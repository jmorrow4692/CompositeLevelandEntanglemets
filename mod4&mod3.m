...
ContainsKerOfReduction := function(H)
	bool := false;
	pn := #BaseRing(H);
	p := Factorization(pn)[1,1];
	pn_1 :=p^(Factorization(pn)[1,2]-1);
	GLpn := GL(2, ResidueClassRing(pn));
	GLpn_1 := GL(2, ResidueClassRing(pn_1));
	
	//The following is the size of the kernel of reduction
	SizeOfKer := p^4;
	counter:= 0;
	for g in H do
		if (BaseRing(GLpn_1)!(1-g[1,1]) eq 0) and (BaseRing(GLpn_1)!g[1,2] eq 0) and (BaseRing(GLpn_1)!g[2,1] eq 0) and (BaseRing(GLpn_1)!(1-g[2,2]) eq 0) then
			counter := counter +1;
		end if;
	end for;
	if counter eq SizeOfKer then
		bool := true;
	end if;
	return bool;
end function;
...
//X_{H_10,G}(12) does not have any points. Need to check on this
...
//X_{H_23, G_4}(12) has 3 CM points
...
//X_{H_11, G_3}(12) has 6 CM points and 1 cuspidal point
...
//X_{H_12, G_3}(12) has only a cuspidal point
...
//X_{H_24,G_3}(12) has only 1 cuspidal point
...
/*----------------------------X_{H_9, G_2}(12)----------------------------------*/
jmap(9);
C92:= Curve(A2, s^3*(t^2 + 64) - ((t^6 + 144*t^4 + 6912*t^2 + 110592)*27*(s+1)^3*(s-3)^3));
CC92 := Curve(A2, s^3*(t + 64) - ((t^3 + 144*t^2 + 6912*t^1 + 110592)*27*(s+1)^3*(s-3)^3));
assert (Genus(CC92) eq 2);
boo, H92, phi := IsHyperelliptic(CC92);
assert (boo eq true);
HH92,phi2 := SimplifiedModel(H92);
J := Jacobian(HH92);
RankBound(J);

// Need to do etale descent
...
// X_{H_13, G_2}(12) has 2 cuspidal points
